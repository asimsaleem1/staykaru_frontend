...
const DASHBOARD_API = 'https://staykaru-backend-60ed08adb2a7.herokuapp.com/api/admin/dashboard';
...
const DashboardScreen = () => {
  ...
  const fetchDashboard = async () => {
    ...
    try {
      ...
      console.log('🔍 DEBUG - Current Token:', storedToken ? 'Present' : 'Missing');
      console.log('🔍 DEBUG - Current User:', storedUser ? JSON.parse(storedUser) : 'No user data');
      ...
      console.log('🔍 DEBUG - Making request to:', DASHBOARD_API);
      console.log('🔍 DEBUG - Headers:', headers);
      
      const res = await fetch(DASHBOARD_API, { headers, credentials: 'include' });
      ...
      if (res.ok) {
        ...
        if (data.stats) {
          // Format: { stats: { totalUsers: 10, ... } }
          dashboardStats = data.stats;
        } else if (data.totalUsers !== undefined || data.totalAccommodations !== undefined) {
          // Format: { totalUsers: 10, totalAccommodations: 5, ... }
          dashboardStats = data;
        } else if (data.data && data.data.stats) {
          // Format: { data: { stats: { ... } } }
          dashboardStats = data.data.stats;
        } else if (data.users !== undefined || data.accommodations !== undefined) {
          // Format: { users: [...], accommodations: [...], ... }
          dashboardStats = {
        ...
        console.log('📊 Processed stats:', dashboardStats);
        setStats(dashboardStats);
        
        // Set system status based on response
        setSystemStatus('healthy');
        
        // Mock recent activity for now
        setRecentActivity([
          { id: 1, type: 'user', message: 'New user registered', time: '2 minutes ago', icon: 'person-add', color: '#4F8EF7' },
          { id: 2, type: 'booking', message: 'New booking created', time: '5 minutes ago', icon: 'calendar', color: '#FF9500' },
          { id: 3, type: 'accommodation', message: 'New accommodation listed', time: '10 minutes ago', icon: 'home', color: '#34C759' },
          { id: 4, type: 'order', message: 'Food order placed', time: '15 minutes ago', icon: 'fast-food', color: '#FF2D55' },
          { id: 5, type: 'payment', message: 'Payment received', time: '20 minutes ago', icon: 'card', color: '#30D158' },
        ]);
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          <View style={styles.errorContainer}>
            ...
            <View style={styles.debugContainer}>
              ...
              <Text style={styles.debugText}>User Role: {userData?.role || 'Unknown'}</Text>
              <Text style={styles.debugText}>User Email: {userData?.email || 'Unknown'}</Text>
            </View>
            ...
            <TouchableOpacity style={styles.logoutBtn} onPress={handleLogout}>
              ...
              <Text style={styles.logoutText}>Logout & Try Different Account</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      </SafeAreaView>
