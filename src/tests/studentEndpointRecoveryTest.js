// Comprehensive Student Endpoint Recovery Test - Finding Working Alternatives
const fetch = require('node-fetch');

const API_BASE_URL = 'https://staykaru-backend-60ed08adb2a7.herokuapp.com/api';

// Using a fresh admin token for testing
...
async function testFailedEndpointsWithAlternatives() {
    console.log('🔧 Testing FAILED student endpoints with alternative solutions...');
    ...
    for (const test of failedTests) {
        ...
        for (const alternative of test.alternatives) {
            try {
                ...
                if (response.ok) {
                    console.log(`✅ ${test.name} -> ${alternative.method} ${alternative.url} (${response.status})`);
                    ...
                    console.log(`⚠️ ${test.name} -> ${alternative.method} ${alternative.url} (${response.status})`);
                }
            } catch (error) {
        ...
        if (!found) {
            console.log(`❌ ${test.name} -> All alternatives failed`);
    ...
    console.log(`📊 Total Failed Endpoints: ${failedTests.length}`);
    console.log(`✅ Recovered: ${workingEndpoints.length}`);
    console.log(`❌ Still Failing: ${stillFailingEndpoints.length}`);
    console.log(`📈 Recovery Rate: ${((workingEndpoints.length / failedTests.length) * 100).toFixed(1)}%`);

    if (workingEndpoints.length > 0) {
        console.log('\n🎯 RECOVERED ENDPOINTS:');
        workingEndpoints.forEach(endpoint => {
            console.log(`✅ ${endpoint.name} -> ${endpoint.method} ${endpoint.endpoint} (${endpoint.status})`);
        });
    }
    ...
    return { workingEndpoints, stillFailingEndpoints };
}

testFailedEndpointsWithAlternatives().catch(console.error);
